...
def _supply_index(block_size, number_write=0, remainder=0, mode='forth'):
...
def _write_file(file_path=None, file_size=8192, data_pattern='fixed', \
    ...
    :param rewrite      : indicates if overwrite existing files
    ...
    index_supplier = _supply_index(gblocksize, numwrite, remainder, mode)
    if remainder > 0:
...
def _read_file(path=None, block_size=8192, io_mode='forth'):
    ...
    index_supplier = _supply_index(blocksize, numread, remainder, mode)
    if remainder > 0:
...
def _advance_compress_file(file_path=None, file_size=8192, block_size=8192, \
        pattern_length=4, number_pattern=1, data_check=False, rewrite=False, mode='forth'):
    ...
    index_supplier = _supply_index(blocksize, numwrite, remainder, mode)
    if remainder > 0:
        rindex = next(index_supplier) # get index of remainder
    with open(filepath, openmode) as f:
        try:
...
def _compress_file(file_path=None, file_size=8192, block_size=8192, compress_percent=50,\
        compress_pattern='\x00', data_check=False, rewrite=False, mode='forth'):
...
def _create_link(path=None):
    ...
    if ghardlinkcount <= 0 and gsymlinkcount <= 0:
        logger.info("create link done or no need to create link")
        return
    prefixre = re.escape(gnameprefix)
    # tree root first
    if gnameprefix != '':
...
if __name__ == "__main__":
    ...
    while iteration > 0:
        ...
        for directory, destdirectory, action, width, level, numfile, \
            ...
            seektype, user in zip(directoryqueue, destdirectoryqueue, \
            ...
            datapatternqueue, seektypequeue, userqueue):
            ...
            # action rewrite
                        ...
                        try:
                            if patternpercent > 0 and datapattern == 'sparse':
                                ...
                                _compress_file(f, filesize, gblocksize, \
                                        patternpercent, compresspattern, datacheck, grewrite, gseektype)
                            elif datapattern == 'complex-compress':
                                _advance_compress_file(f, filesize, gblocksize, \
                                        gcompresspatternlen, gnumcompresspattern, datacheck, grewrite, gseektype)
                            else:
                                _write_file(f, filesize, datapattern, datacheck, grewrite, gseektype)
                            gfilecount += 1 
                        except IOError as e:
                    ...
                    for f in _inspect_tree(directory, 'file'):
                        ...
                        try:
                            if patternpercent > 0 and datapattern == 'sparse':
                                ...
                                _compress_file(f, filesize, gblocksize, \
                                        patternpercent, compresspattern, datacheck, grewrite, seektype)
                            elif datapattern == 'complex-compress':
                                _advance_compress_file(f, filesize, gblocksize, \
                                        gcompresspatternlen, gnumcompresspattern, datacheck, grewrite, seektype)
                            elif datapattern == 'fixed' \
                                    or datapattern == 'ZERO' \
                                    or datapattern == 'ONE' \
                                    or datapattern == 'random':
                                _write_file(f, filesize, datapattern, datacheck, grewrite, seektype)
                            gfilecount += 1 
                        except IOError as e:
                            logger.error("Exception caught while rewriting file: %s (%s)" % (f, str(e)))
                            raise IOError(str(e))
                ...
                if gnumfilepercent == 100:
                    for f in _traverse_tree(directory, 'file'):
                        try: 
                            _read_file(f, gblocksize, gseektype)
                            gfilecount += 1
                        except Exception as e:
